...
include dirname(__DIR__)."/page-master/model/action.php";
...
$constructs ="SELECT * FROM page WHERE $construct";
...
if(!$start)
$start=0; 
$getquery = mysqli_query($conn, "SELECT * FROM page WHERE $construct LIMIT 0,5");
  
while($runrows = mysqli_fetch_assoc($getquery))
{
$title = $runrows ['title'];
$desc = $runrows ['create_date'];
$url = $runrows ['status'];
